...
When DisCov values are reported -- for instance, in <span class="artifact-n">[bam-stats-txt](/help/main/artifacts/bam-stats-txt)</span> -- usually _S_, _E_, and the number of windows are individually reported in addition to the overall DisCov score.
...
<span class="artifact-p">[anvi-profile-blitz](/help/main/programs/anvi-profile-blitz)</span> always computes DisCov scores as part of its output. <span class="artifact-p">[anvi-summarize](/help/main/programs/anvi-summarize)</span> computes DisCov scores when the `--report-discov` flag is provided.
...
The per-base coverage arrays are either obtained directly from a <span class="artifact-n">[bam-file](/help/main/artifacts/bam-file)</span>, or from the auxiliary database files (`AUXILIARY-DATA.db`) produced by <span class="artifact-p">[anvi-profile](/help/main/programs/anvi-profile)</span>.
...
{:.notice}
Edit [this file](https://github.com/merenlab/anvio/tree/master/anvio/docs/artifacts/discov-stats.md) to update this information.
