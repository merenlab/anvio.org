...
For every KO in the path above that has a hit in the <span class="artifact-n">[contigs-db](/help/main/artifacts/contigs-db)</span>, there will be a corresponding line in the 'hits_in_modules' output file. The same will occur for every possible path in every single KEGG module, resulting in a lot of lines and extremely repetitive but nicely parseable information.
