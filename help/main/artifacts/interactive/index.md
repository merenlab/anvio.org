...
![search view](../../images/interactive_interface/search_view.png){:.center-img .width-70}
  ...
  - __Highlight the search__: you can highlight the items matching the search by clicking, change the color of the highlight and remove any existing highlight. When the highlight comes from a __Search functions__ query, opening one of those splits in the inspect page (<span class="artifact-p">[anvi-inspect](/help/main/programs/anvi-inspect)</span>) will also highlight the gene(s) that matched, so you can spot them at a glance instead of clicking through every gene.
