---
layout: program
title: anvi-reorient-genomes
excerpt: An anvi'o program. Reorient circular genomes and scaffold fragmented genomes in a fasta-txt so their coordinates match a chosen reference genome using minimap2 alignment and seqkit manipulation.
categories: [anvio]
comments: false
redirect_from: /m/anvi-reorient-genomes
image:
...
<p style="text-align: left" markdown="1"><span class="artifact-r">[fasta-txt](../../artifacts/fasta-txt) <img src="../../images/icons/TXT.png" class="artifact-icon-mini" /></span></p>
...
<span class="artifact&#45;p">[anvi&#45;reorient&#45;genomes](/help/main/programs/anvi&#45;reorient&#45;genomes)</span> &#45;&#45;fasta&#45;txt <span class="artifact&#45;n">[fasta&#45;txt](/help/main/artifacts/fasta&#45;txt)</span> \
...
<div class="codeblock" markdown="1">
anvi&#45;reorient&#45;genomes &#45;&#45;fasta&#45;txt <span class="artifact&#45;n">[fasta&#45;txt](/help/main/artifacts/fasta&#45;txt)</span> \
...
Finally, please note that all of this applies **only to the reference**. Every *other* entry in your <span class="artifact-n">[fasta-txt](/help/main/artifacts/fasta-txt)</span> file is free to be as fragmented as they come.
...
By default, <span class="artifact-p">[anvi-reorient-genomes](/help/main/programs/anvi-reorient-genomes)</span> will rotate the automatically chosen reference genome to an optimal starting position that is conserved across most of the input genomes mentioned in the <span class="artifact-n">[fasta-txt](/help/main/artifacts/fasta-txt)</span>: it will first align each genome to the reference **using all possible good alignments (primary + secondary alignments)** to get a true picture of conserved regions across all genomes, and then it will use a 1,000 bp sliding window to search for a region that is shared between all genomes in a greedy fashion (once it finds a region that works for all genomes, it stops).
...
Since the reference must be a single contig, a <span class="artifact-n">[fasta-txt](/help/main/artifacts/fasta-txt)</span> file in which *every* entry is fragmented (a set of MAGs, say) will make the program stop with an error. You have two options.

**The good option** is to add a single-contig reference to your <span class="artifact-n">[fasta-txt](/help/main/artifacts/fasta-txt)</span> file and point anvi'o to it:
...
<span class="artifact-p">[anvi-reorient-genomes](/help/main/programs/anvi-reorient-genomes)</span> will then align each fragment to the reference on their own merit, ordering and orienting them like any other contig, and reporting them individually.
...
<span class="artifact-p">[anvi-reorient-genomes](/help/main/programs/anvi-reorient-genomes)</span> will correct such contigs since they completely violate the synteny of genes in the actual genomic context. But it will require both of these to be correct before touching such a contig:
...
10. **Write outputs**. Copies the reference <span class="artifact-n">[fasta](/help/main/artifacts/fasta)</span> to the output directory (potentially rotated if auto-selected). Writes each reoriented query <span class="artifact-n">[fasta](/help/main/artifacts/fasta)</span> under the same name (and using the original extension).
...
{:.notice}
Edit [this file](https://github.com/merenlab/anvio/tree/master/anvio/docs/programs/anvi-reorient-genomes.md) to update this information.
...
{:.notice}
Are you aware of resources that may help users better understand the utility of this program? Please feel free to edit [this file](https://github.com/merenlab/anvio/blob/master/anvio/cli/reorient_genomes.py) on GitHub. If you are not sure how to do that, find the `__resources__` tag in [this file](https://github.com/merenlab/anvio/blob/master/anvio/cli/interactive.py) to see an example.
