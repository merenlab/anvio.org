...
This program **helps you analyze and understand contigs in one or more <span class="artifact-n">[contigs-db](/help/main/artifacts/contigs-db)</span>s**.

### Working with single or multiple contigs databases

You can use this program on a single contigs database as follows:

<div class="codeblock" markdown="1">
anvi&#45;display&#45;contigs&#45;stats CONTIGS&#45;01.db
</div>

Alternatively, you can use it to compare multiple contigs databases:
...
When comparing multiple databases, each contigs database will become an individual column in all outputs.
...
it will launch an interactive interface that looks like this:
...
* The bars in the upper graph represent every integer N and L statistic from 1 to 100. The y-axis shows the respective N length and the x-axis shows the percentage of the total dataset examined (the exact L and N values can be seen by hovering over each bar). In other words, if you sorted your contigs by length (from longest to shortest) and walked through each one, every time you had examined another 1 percent of your total dataset, you would add a bar to the graph showing the number of contigs that you had seen (the L statistic) and the length of the contig you were currently examining (the N statistic).

* The lower portion of the graph displays information about which HMM hits your contigs database contains. Each column represents a gene in a specific <span class="artifact-n">[hmm-source](/help/main/artifacts/hmm-source)</span>, and the graph shows how many hits each gene has in your data. (Hover your mouse over the graph to see the specifics of each gene.) The sidebar shows how many genes in this graph were detected exactly that many times. For example, in the graph above, for the Bacteria_71 <span class="artifact-n">[hmm-source](/help/main/artifacts/hmm-source)</span>, many genes were detected 9-11 times, so those bars are longer. This helps you estimate approximately how many of these genomes are present in your contigs database (so here, there are likely around 9-11 bacterial genomes in this contigs database). See the section about [predicting number of genomes](#how-do-we-predict-the-number-of-genomes) for more details.
...
- The number of contigs of varying lengths. (for example "Num Contigs > 2.5 kb" shows the number of contigs that are longer than 2500 base pairs)
- The length of the longest and shortest contigs in your database in nucleotides
- The number of genes in your contigs (as predicted by [Prodigal](https://github.com/hyattpd/Prodigal))
- L50, L75, L90: If you ordered the contigs in your database from longest to shortest, these stats describe the *number of contigs* you would need to examine before you had looked at a certain percentage of the entire genome. For example, L50 describes the number of contigs you would have to examine before you reached 50 percent of the entire dataset.
- N50, N75, N90: If you ordered the contigs in your database from longest to shortest, these stats describe the *length of the contig* you would be examining when you had looked at a certain percentage of the entire genome. For example, N50 describes the length of the contig you would be examining when you reached 50 percent of the entire genome length.
- The number of HMM hits in your contigs. This goes through every <span class="artifact-n">[hmm-source](/help/main/artifacts/hmm-source)</span> and provides the number of hits its genes had across all of your contigs. Essentially, this is the number of hits that is shown in the lower graph at the top of the page.
- The number of genomes that anvi'o predicts are present in your sample, based on how many hits the single-copy core genes received from the various <span class="artifact-n">[hmm-source](/help/main/artifacts/hmm-source)</span>s. See the section about [predicting number of genomes](#how-do-we-predict-the-number-of-genomes) below.
...
There is also another flag you can add to get the output formatted as markdown, which makes it easier to copy-paste to GitHub or other markdown-friendly services. This is how you get markdown output instead:
...
You can easily convert the markdown output into PDF or HTML pages using [pandoc](https://pandoc.org/). For instance running the following command on the previous output:
...
will result in a PDF file that looks like this:
