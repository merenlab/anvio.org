...
categories: [anvio]
comments: false
redirect_from: /8/trnaseq-workflow
image:
...
The tRNA-seq workflow **is a [Snakemake](https://snakemake.readthedocs.io/en/stable/) workflow run by the anvi'o script, <span class="artifact-p">[anvi-run-workflow](/help/8/programs/anvi-run-workflow)</span>**.
...
The tRNA-seq workflow requires two files to run: a json-formatted config file and <span class="artifact-n">[samples-txt](/help/8/artifacts/samples-txt)</span>. Generate the default config file, here called `config.json`, with the following command.
