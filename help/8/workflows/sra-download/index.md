...
The first step is to make a <span class="artifact-n">[workflow-config](/help/8/artifacts/workflow-config)</span>.

```bash
anvi-run-workflow -w sra-download --get-default-config sra_download_config.json
```

Here's what the <span class="artifact-n">[workflow-config](/help/8/artifacts/workflow-config)</span> file looks like:
