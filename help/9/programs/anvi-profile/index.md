...
This program **creates a <span class="artifact-n">[single-profile-db](/help/9/artifacts/single-profile-db)</span> from a <span class="artifact-n">[bam-file](/help/9/artifacts/bam-file)</span> and <span class="artifact-n">[contigs-db](/help/9/artifacts/contigs-db)</span>**.

Once you have a <span class="artifact-n">[single-profile-db](/help/9/artifacts/single-profile-db)</span>, you can run programs like <span class="artifact-p">[anvi-cluster-contigs](/help/9/programs/anvi-cluster-contigs)</span>, <span class="artifact-p">[anvi-estimate-metabolism](/help/9/programs/anvi-estimate-metabolism)</span>, and <span class="artifact-p">[anvi-gen-gene-level-stats-databases](/help/9/programs/anvi-gen-gene-level-stats-databases)</span>, as well as use the interactive interface with <span class="artifact-p">[anvi-interactive](/help/9/programs/anvi-interactive)</span>. If you want to run these same contigs against multiple BAM files (because you have multiple samples), you'll combine your <span class="artifact-n">[single-profile-db](/help/9/artifacts/single-profile-db)</span>s into a <span class="artifact-n">[profile-db](/help/9/artifacts/profile-db)</span> after you've created them all using <span class="artifact-p">[anvi-merge](/help/9/programs/anvi-merge)</span>. See the pages for <span class="artifact-n">[single-profile-db](/help/9/artifacts/single-profile-db)</span> or <span class="artifact-n">[profile-db](/help/9/artifacts/profile-db)</span> for more you can do with these artifacts.

In short, this program runs various analyses on the contigs in your <span class="artifact-n">[contigs-db](/help/9/artifacts/contigs-db)</span> and how they relate to the sample information stored in the <span class="artifact-n">[bam-file](/help/9/artifacts/bam-file)</span> you provided. It then stores this information into a <span class="artifact-n">[single-profile-db](/help/9/artifacts/single-profile-db)</span>. Specifically, this program calculates
...
This program takes in an [indexed](https://merenlab.org/software/anvio/help/programs/anvi-init-bam) <span class="artifact-n">[bam-file](/help/9/artifacts/bam-file)</span> and a <span class="artifact-n">[contigs-db](/help/9/artifacts/contigs-db)</span>. The BAM file contains the short reads from a single sample that will be used to create the profile database. Thus, here is a standard run with default parameters:
...
<div class="codeblock" markdown="1">
anvi&#45;profile &#45;c <span class="artifact&#45;n">[contigs&#45;db](/help/9/artifacts/contigs&#45;db)</span>  \
            &#45;&#45;blank&#45;profile
</div>
...
By default, anvi'o will use every contig in your <span class="artifact-n">[contigs-db](/help/9/artifacts/contigs-db)</span>. However, if you wish to focus specifically on a subset of these contigs, just provide a file that contains only the names of the contigs you want to analyze, one per line, using the tag `--contigs-of-interest`.
...
Keep in mind that if you plan to merge your resulting <span class="artifact-n">[single-profile-db](/help/9/artifacts/single-profile-db)</span> with others later in the project, you'll want to keep these parameters consistent.
...
For instance, if you wish to work only with contigs that have more than a certain coverage across your samples, you can first run the program <span class="artifact-p">[anvi-profile-blitz](/help/9/programs/anvi-profile-blitz)</span> on all BAM files, then use the resulting output file <span class="artifact-n">[bam-stats-txt](/help/9/artifacts/bam-stats-txt)</span> to identify contigs of interest based on their coverages across samples, then put their names in a text file, and pass this file to <span class="artifact-p">[anvi-profile](/help/9/programs/anvi-profile)</span> using the flag `--contigs-of-interest` (the anvi'o profile used to have a flag for this, `--min-mean-coverage`, that allowed users to remove contigs based on their coverage in a given sample, but [we recently removed it](https://github.com/merenlab/anvio/issues/2047) to promote explicit specification of contigs.
...
If you're planning to later merge this sample with others, it is better to perform clustering while running <span class="artifact-p">[anvi-merge](/help/9/programs/anvi-merge)</span> than at this stage.
...
{:.notice}
Are you aware of resources that may help users better understand the utility of this program? Please feel free to edit [this file](https://github.com/merenlab/anvio/blob/master/anvio/cli/profile.py) on GitHub. If you are not sure how to do that, find the `__resources__` tag in [this file](https://github.com/merenlab/anvio/blob/master/anvio/cli/interactive.py) to see an example.
