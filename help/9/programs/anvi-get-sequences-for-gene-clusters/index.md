...
categories: [anvio]
comments: false
redirect_from: /9/anvi-get-sequences-for-gene-clusters
image:
...
This aptly-named program **gets the sequences for the gene clusters stored in a <span class="artifact-n">[pan-db](/help/9/artifacts/pan-db)</span> and returns them as either a <span class="artifact-n">[genes-fasta](/help/9/artifacts/genes-fasta)</span> or a <span class="artifact-n">[concatenated-gene-alignment-fasta](/help/9/artifacts/concatenated-gene-alignment-fasta)</span>**, which can directly go into the program <span class="artifact-p">[anvi-gen-phylogenomic-tree](/help/9/programs/anvi-gen-phylogenomic-tree)</span> for phylogenomics. This gives you advanced access to your gene clusters, so you can take them out of anvi'o and do whatever you please with them.

The program parameters also include a large collection of advanced filtering options. Using these options you can scrutinize your gene clusters in creative and precise ways. Using the combination of these filters you can focus on single-copy core gene clusters in a pangenome, or those occur only as singletons, or paralogs that contain more than a given number of sequences, and so on. Once you are satisfied with the output a given set of filters generate, you can add the matching gene clusters a <span class="artifact-n">[misc-data-items](/help/9/artifacts/misc-data-items)</span> with the flag `--add-into-items-additional-data-table`, which can be added to the <span class="artifact-n">[interactive](/help/9/artifacts/interactive)</span> interface as additional layers when you visualize your <span class="artifact-n">[pan-db](/help/9/artifacts/pan-db)</span> using the program <span class="artifact-p">[anvi-display-pan](/help/9/programs/anvi-display-pan)</span> 

By default, <span class="artifact-p">[anvi-get-sequences-for-gene-clusters](/help/9/programs/anvi-get-sequences-for-gene-clusters)</span> will generate a single output file. But you can ask the program to report every gene cluster that match to your filters as a separate FASTA file depending on your downstream analyses.
...
Here is an example that shows the simplest possible run, which will export sequences for every single gene cluster found in the <span class="artifact-n">[pan-db](/help/9/artifacts/pan-db)</span> as amino acid sequences:

<div class="codeblock" markdown="1">
anvi&#45;get&#45;sequences&#45;for&#45;gene&#45;clusters &#45;g <span class="artifact&#45;n">[genomes&#45;storage&#45;db](/help/9/artifacts/genomes&#45;storage&#45;db)</span> \
                                     &#45;p <span class="artifact&#45;n">[pan&#45;db](/help/9/artifacts/pan&#45;db)</span> \
                                     &#45;o <span class="artifact&#45;n">[genes&#45;fasta](/help/9/artifacts/genes&#45;fasta)</span>
</div>
...
You can export only the sequences for a specific <span class="artifact-n">[collection](/help/9/artifacts/collection)</span> or <span class="artifact-n">[bin](/help/9/artifacts/bin)</span> with the parameters `-C` or `-b` respectively. 
...
{:.notice}
You can always display the collections and bins available in your <span class="artifact-n">[pan-db](/help/9/artifacts/pan-db)</span> by adding `--list-collections` or `--list-bins` flags to your command.
...
For example, the following run on a <span class="artifact-n">[genomes-storage-db](/help/9/artifacts/genomes-storage-db)</span> that contains 50 genomes will report only the single-copy core genes with a functional homogenity index above 0.25:
...
To get a <span class="artifact-n">[concatenated-gene-alignment-fasta](/help/9/artifacts/concatenated-gene-alignment-fasta)</span> (which you can use to run <span class="artifact-p">[anvi-gen-phylogenomic-tree](/help/9/programs/anvi-gen-phylogenomic-tree)</span>), use the parameter `--concatenate-gene-clusters`
...
{:.notice}
Are you aware of resources that may help users better understand the utility of this program? Please feel free to edit [this file](https://github.com/merenlab/anvio/blob/master/anvio/cli/get_sequences_for_gene_clusters.py) on GitHub. If you are not sure how to do that, find the `__resources__` tag in [this file](https://github.com/merenlab/anvio/blob/master/anvio/cli/interactive.py) to see an example.
