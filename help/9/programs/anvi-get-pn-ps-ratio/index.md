...
categories: [anvio]
comments: false
redirect_from: /9/anvi-get-pn-ps-ratio
image:
...
This program **calculates the pN/pS ratio** for each gene in a <span class="artifact-n">[contigs-db](/help/9/artifacts/contigs-db)</span> and outputs it as a <span class="artifact-n">[pn-ps-data](/help/9/artifacts/pn-ps-data)</span> artifact.
...
First, you will need to run <span class="artifact-p">[anvi-gen-variability-profile](/help/9/programs/anvi-gen-variability-profile)</span> using the flag `--engine CDN` to get a <span class="artifact-n">[variability-profile-txt](/help/9/artifacts/variability-profile-txt)</span> for SCVs (single codon variants), which we'll name `SCVs.txt` in this example.
...
A pN/pS value is calculated for each gene x sample combo. This will result in a directory called `output_dir` that contains several tables that describe each of your genes. See <span class="artifact-n">[pn-ps-data](/help/9/artifacts/pn-ps-data)</span> for more information.
