...
categories: [anvio]
comments: false
redirect_from: /9/anvi-display-contigs-stats
image:
...
This program **helps you make sense of contigs in one or more <span class="artifact-n">[contigs-db](/help/9/artifacts/contigs-db)</span> files**, summarizing contig lengths, gene lengths, HMM hits, and single-copy core gene (SCG) based genome estimates.
...
If you are comparing multiple, each contigs databse will become an individual column in all outputs (columns are labeled with the `project_name` stored in each <span class="artifact-n">[contigs-db](/help/9/artifacts/contigs-db)</span>).
...
<div class="codeblock" markdown="1">
anvi&#45;display&#45;contigs&#45;stats <span class="artifact&#45;n">[contigs&#45;db](/help/9/artifacts/contigs&#45;db)</span>
</div>

it will open an interactive interface that displays all the summary data. Here is an example with a few <span class="artifact-n">[contigs-db](/help/9/artifacts/contigs-db)</span> files and the output it displays:
...
* **Gene stats** (using the default gene caller recorded in the <span class="artifact-n">[contigs-db](/help/9/artifacts/contigs-db)</span>, e.g., Prodigal or pyrodigal-gv)
  - Num Genes
  - Avg Gene Length
  - Avg Gene Length (trim 10%): two-sided 10% trimmed mean of gene lengths
  - Min Gene Length / Max Gene Length
* **HMM hits**
  - For every <span class="artifact-n">[hmm-source](/help/9/artifacts/hmm-source)</span> found in the <span class="artifact-n">[contigs-db](/help/9/artifacts/contigs-db)</span> (including SCG sets and other sources such as `Ribosomal_RNAs`), the total number of hits; `n/a` is shown for contigs databases where a source is missing.
* **SCG-based estimates for number of populations**
  - For each <span class="artifact-n">[hmm-source](/help/9/artifacts/hmm-source)</span> used for SCGs (for example `Bacteria_71`, `Archaea_76`, `Protista_83`), the predicted number of genomes. See [predicting number of genomes](#how-do-we-predict-the-number-of-genomes).
...
The SCGs for each domain can be annotated in a <span class="artifact-n">[contigs-db](/help/9/artifacts/contigs-db)</span> with <span class="artifact-p">[anvi-run-hmms](/help/9/programs/anvi-run-hmms)</span>, which utilizes three domain-specific SCG sets: the <span class="artifact-n">[hmm-source](/help/9/artifacts/hmm-source)</span>s `Bacteria_71`, `Archaea_76`, and `Protista_83`. So what we do is get an estimate for each of these domains, which are the predicted number of genomes that you see in the stats table:
