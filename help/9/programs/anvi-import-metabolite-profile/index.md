...
categories: [anvio]
comments: false
redirect_from: /9/anvi-import-metabolite-profile
image:
...
This program imports a metabolite abundance profile, such as from metabolomic experiments, into a <span class="artifact-n">[profile-db](/help/9/artifacts/profile-db)</span>.

This program takes as input a tab-delimited file of metabolite abundance data and a <span class="artifact-n">[profile-db](/help/9/artifacts/profile-db)</span>. The tabular file must have three columns with the following names: "accession", "sample", and "abundance". Each row of the table corresponds to a distinct metabolite abundance measurement.
...
Once metabolite abundances are stored in a profile database, they can be loaded into a metabolic <span class="artifact-n">[reaction-network](/help/9/artifacts/reaction-network)</span> for analysis in the context of biochemical pathways. Metabolites in the network are defined in terms of ModelSEED Compounds.
