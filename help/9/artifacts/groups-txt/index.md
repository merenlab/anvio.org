...
Here is an example <span class="artifact-n">[groups-txt](/help/9/artifacts/groups-txt)</span> file:
...
{:.warning}
If you are passing this file to the program <span class="artifact-p">[anvi-compute-metabolic-enrichment](/help/9/programs/anvi-compute-metabolic-enrichment)</span>, the names in the `sample` column must match those in the "modules" mode output file that you provide to the program via the `--modules-txt` parameter. If you know that the sample names match but you are still getting errors, you might need to specify which column in the "modules" mode output contains those sample names using the `--sample-header` parameter.
