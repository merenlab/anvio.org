...
* Run {% include PROGRAM name="anvi-setup-modelseed-database" %}, so that {% include PROGRAM name="anvi-reaction-network" %} can link enzymes to metabolic compounds with ModelSEED data.
* Optimally you can also run `anvi-self-test --suite pangenomics` to see if everything is order, especially if you plan to use anvi'o for pangenomics.
